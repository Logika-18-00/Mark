...
class Area():
    ...
    def outline(self,thinkss,outcolor):
        pygame.draw.rect(screen,outcolor,self.rect,thinkss)
...
class Text(Area):
    def set_text(self, text, fsize =12, text_color=BLACK, shift_x=0,shift_y=0):
        self.text = text
...
while not done:
    ...
    if game.state == "gameover":
        screen.blit(text_game_over, [450, 220])
        screen.blit(text_game_over1, [470, 340])
    pygame.display.flip()
    clock.tick(fps)
